...
app.config["UPLOAD_FOLDER"] = r'D:\minipro\HealthMate-main\miniproject\static\files'
db = SQLAlchemy(app)
bcrypt = Bcrypt(app)

app.config['MAIL_SERVER'] = 'smtp.office365.com'
app.config['MAIL_PORT'] = 587
app.config['MAIL_USERNAME'] = 'healthmateminiproject@outlook.com'
...
def register():
    if request.method == "POST":
        if UserDB.query.filter_by(userName=request.form["un"]).first() is None:
            ...
            if len(userpwd)<8:
                return render_template("register.html", x=4)
            if not any(char in SpecialSym for char in userpwd):
                return render_template("register.html", x=5)
    ...
    if request.method == "POST":
        ...
        if not user:
            ...
            msg = Message(
                ...
                sender='healthmateminiproject@outlook.com',
